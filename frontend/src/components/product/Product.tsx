...
const Product: FC<IProduct> = (item) => {
	...
	const addToCart = () => {
		...
		if (getTotalQuantityInCart(cart) >= 200) {
			toast.warning(
				<>
					<div>Превышен лимит товаров в корзине</div>
					<div className="toast-details">Максимальное количество: 200 шт.</div>
				</>
			);
			return;
		}

		const availableQuantity = item.quantity - currentQuantityInCart;
		if (availableQuantity <= 0) {
			toast.warning(
				<>
					<div>Недостаточно товаров на складе</div>
					<div className="toast-details">Доступно: {quantity} шт.</div>
				</>
			);
			return;
		}

		dispatch(addItemToCart({ ...item, quantity: currentQuantityInCart + 1 }));
												...
												if (quantity > 0) {
													...
													toast.warning(
														<>
															...
															<div className="toast-details">
																Пожалуйста, дождитесь поступления на склад
															</div>
														</>
													);
												}
